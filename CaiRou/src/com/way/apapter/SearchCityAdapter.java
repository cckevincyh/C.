...
public class SearchCityAdapter extends BaseAdapter implements Filterable {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		provinceTv.setText(mResultCities.get(position).getCity_province());
		TextView cityTv = (TextView) convertView
				.findViewById(R.id.column_title);
		cityTv.setText(mResultCities.get(position).getCity_area());
		return convertView;
	}
	...
	public Filter getFilter() {
		Filter filter = new Filter() {
			...
			protected FilterResults performFiltering(CharSequence s) {
				String str = s.toString().toLowerCase();//搜索栏的
				// mFilterStr = str;
				...
				if (mAllCities != null && mAllCities.size() != 0) {
					for (City cb : mAllCities) {
						// 匹配全屏、首字母、和城市名中文
						if (cb.getCity_spell_zh().indexOf(str) > -1
								|| cb.getCity_area().indexOf(str) > -1) {
							cityList.add(cb);
						}
					}
				}
